...
class TestFilteredQueue:
    ...
    async def test_filtered_queue_list_input(self):
        ...
        mock_filter = Mock(side_effect=lambda x, **kwargs: {**x, "processed": True})
        ...
        assert results == [{"id": 1, "processed": True}, {"id": 2, "processed": True}]
    ...
    def test_filtered_queue_put_nowait(self):
...
class TestHeartbeatTask:
    ...
    async def test_heartbeat_task(self):
        ...
        with patch("ansible_rulebook.engine.send_session_stats") as mock_send_stats:
            with patch("ansible_rulebook.engine.session_stats") as mock_session_stats:
    ...
    async def test_heartbeat_task_cancellation(self):
        ...
        task = asyncio.create_task(
            heartbeat_task(event_log, rule_set_names, 0.1)
        )
...
class TestMonitorRulebook:
    ...
    async def test_monitor_rulebook_file_changed(self, mock_observer_class):
        """Test monitor_rulebook raises HotReloadException when file changes."""
        mock_observer = Mock()
        ...
        with patch("ansible_rulebook.engine.RulebookFileChangeHandler", return_value=handler):
    ...
    async def test_monitor_rulebook_no_changes(self, mock_observer_class):
        ...
        with patch("ansible_rulebook.engine.RulebookFileChangeHandler", return_value=handler):
            ...
            task = asyncio.create_task(monitor_rulebook("/fake/path/rulebook.yml"))
...
class TestStartSource:
    ...
    async def test_start_source_local_file(self):
        ...
        with patch("os.path.exists", return_value=True):
            with patch("ansible_rulebook.engine.has_builtin_filter", return_value=True):
                with patch("ansible_rulebook.engine.find_builtin_filter", return_value="/fake/filter.py"):
                    with patch("ansible_rulebook.engine.runpy.run_path") as mock_run_path:
                        ...
                        # Run start_source
                        await start_source(source, ["/fake/source/dir"], variables, queue)
...
""")
    ...
    async def test_start_source_with_filters(self):
        ...
        # Mock all the dependencies
        with patch("os.path.exists", return_value=True):
            with patch("ansible_rulebook.engine.has_builtin_filter") as mock_has_builtin:
                with patch("ansible_rulebook.engine.find_builtin_filter", return_value="/fake/filter.py"):
                    with patch("ansible_rulebook.engine.runpy.run_path") as mock_run_path:
                        
                        # Configure has_builtin_filter to return True for both filters
                        def has_builtin_side_effect(filter_name):
                            return filter_name in ["test_filter", "eda.builtin.insert_meta_info"]
                        ...
                        await start_source(source, ["/fake/source/dir"], {}, queue)
...
""")
        ...
        try:
            ...
            with patch("ansible_rulebook.engine.has_source_filter", return_value=False):
                with patch("ansible_rulebook.engine.has_builtin_filter", return_value=False):
...
class TestRunRulesets:
    ...
    async def test_run_rulesets_empty_plans(self):
        ...
        with patch("ansible_rulebook.engine.rule_generator.generate_rulesets", return_value=[]):
    ...
    async def test_run_rulesets_basic_execution(self):
        ...
        # Mock dependencies
        with patch("ansible_rulebook.engine.rule_generator.generate_rulesets") as mock_generate:
            with patch("ansible_rulebook.engine.establish_async_channel") as mock_channel:
                with patch("ansible_rulebook.engine.handle_async_messages") as mock_handle:
                    with patch("ansible_rulebook.engine.RuleSetRunner") as mock_runner_class:
                        
                        # Create mock drools ruleset with define method
                        mock_drools_ruleset = Mock()
                        mock_drools_ruleset.define.return_value = "mocked ruleset definition"
                        mock_drools_ruleset.name = "test_ruleset"  # Set the name property
    ...
    async def test_run_rulesets_with_heartbeat(self):
        ...
        with patch("ansible_rulebook.engine.rule_generator.generate_rulesets") as mock_generate:
            with patch("ansible_rulebook.engine.establish_async_channel") as mock_channel:
                with patch("ansible_rulebook.engine.handle_async_messages") as mock_handle:
                    with patch("ansible_rulebook.engine.RuleSetRunner") as mock_runner_class:
                        with patch("ansible_rulebook.engine.heartbeat_task") as mock_heartbeat:
    ...
    async def test_run_rulesets_with_file_monitor(self):
        ...
        with patch("ansible_rulebook.engine.rule_generator.generate_rulesets") as mock_generate:
            with patch("ansible_rulebook.engine.establish_async_channel") as mock_channel:
                with patch("ansible_rulebook.engine.handle_async_messages") as mock_handle:
                    with patch("ansible_rulebook.engine.RuleSetRunner") as mock_runner_class:
                        with patch("ansible_rulebook.engine.monitor_rulebook") as mock_monitor:
                            ...
                            # Just test that monitor_rulebook is called when file_monitor is provided
                            # Don't test the complex hot reload logic due to asyncio mocking complexity
                            ...
                            mock_monitor.assert_called_once_with("/path/to/rulebook.yml")
                            # Should return False (no hot reload detected in this simple test)
    ...
    async def test_run_rulesets_gather_facts(self):
        ...
        with patch("ansible_rulebook.engine.rule_generator.generate_rulesets") as mock_generate:
            with patch("ansible_rulebook.engine.establish_async_channel") as mock_channel:
                with patch("ansible_rulebook.engine.handle_async_messages") as mock_handle:
                    with patch("ansible_rulebook.engine.RuleSetRunner") as mock_runner_class:
                        with patch("ansible_rulebook.engine.collect_ansible_facts") as mock_collect_facts:
                            ...
                            mock_channel.return_value = (Mock(), Mock())
                            mock_handle.return_value = AsyncMock()
                            mock_collect_facts.return_value = {"host1": {"fact": "value"}}
                            ...
                            mock_collect_facts.assert_called_once_with(inventory)
                ...
                with patch("ansible_rulebook.engine.handle_async_messages") as mock_handle:
                    with patch("ansible_rulebook.engine.RuleSetRunner") as mock_runner_class:
                        with patch("ansible_rulebook.engine.collect_ansible_facts") as mock_collect_facts:
                            ...
                            await run_rulesets(event_log, ruleset_queues, variables)
